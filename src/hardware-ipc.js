...
function setupHardwareIPC(hardwareManager, kotManager, billManager) {
   ...
   *
   * Made at first boot and never sent anywhere by itself. A shop's cloud
   * server has a different one, generated by the provisioner, so the two can
   * only be introduced by a person: the shopkeeper copies this and allows it
   * in Settings. Reading it here is the local app showing its own key on its
   * own screen.
   */
  ipcMain.handle('bill:get-printing-key', () => process.env.KIOSK_API_KEY || '');

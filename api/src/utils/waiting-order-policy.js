 ...
 * require the desktop shell's modules, so this is the second copy of
 * src/waiting-order-policy.js - the same arrangement as kot-job-key.js and
 * order-source.js. The till reads `alert` and `reach` to decide what noise to
 * make; the API reads `decide` and is the only half that can actually move an
 * order. tests/one-policy-for-an-unanswered-order.test.js proves the two
 * copies answer identically over every shape, because two copies that disagree
 * would have a shop's alarm and a shop's rule working to different clocks.
 */
...
function decideOnSilence(order, policy, waitingMs) {
  ...
  if (onSilence === 'nothing') {
    return {
      decide: 'nothing',
      reason: 'This shop has not asked for anything to happen by itself.',
    };
  }
  if (!afterMs) {

...
const {
  ...
  EXPORT_FIELDS,
  DEFAULT_VALUES,
} = require('../../../src/constants/variants.constants');

describe('variants.constants', () => {
  test('exports response metadata and collection info', () => {
    ...
    expect(COLLECTION_NAME).toBe('variants');
  ...
  test('exports validation and field maps', () => {
    expect(VALIDATION_RULES).toEqual({
      NAME: { MIN_LENGTH: 1, MAX_LENGTH: 20 },
      SEARCH_QUERY: { MIN_LENGTH: 2 },
      PAGINATION: {
        DEFAULT_PAGE: 1,
        DEFAULT_LIMIT: 5,
        DEFAULT_SORT: '_id',

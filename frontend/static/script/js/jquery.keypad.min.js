...
if(inst.options.showOn==='button'||inst.options.showOn==='both'){var buttonStatus=inst.options.buttonStatus;var buttonImage=inst.options.buttonImage;var trigger=$(inst.options.buttonImageOnly?$('<img src="'+buttonImage+'" alt="'+buttonStatus+'" title="'+buttonStatus+'"/>'):$('<button type="button" title="'+buttonStatus+'"></button>').html(buttonImage===''?inst.options.buttonText:$('<img src="'+buttonImage+'" alt="'+buttonStatus+'" title="'+buttonStatus+'"/>')));elem[inst.options.isRTL?'before':'after'](trigger);trigger.addClass(this._triggerClass).click(function(){if(plugin._keypadShowing&&plugin._lastField===elem[0]){plugin.hide()}else{plugin.show(elem[0])}
return!1})}}
inst.saveReadonly=elem.attr('readonly');elem[inst.options.keypadOnly?'attr':'removeAttr']('readonly',!0).on('setData.'+inst.name,function(event,key,value){inst.options[key]=value}).on('getData.'+inst.name,function(event,key){return inst.options[key]});this._setInput(elem,inst);this._updateKeypad(inst)},_preDestroy:function(elem,inst){if(this._curInst===inst){this.hide()}
elem.siblings('.'+this._appendClass).remove().end().siblings('.'+this._triggerClass).remove().end().prev('.'+this._inlineEntryClass).remove();elem.empty().off('.'+inst.name)[inst.saveReadonly?'attr':'removeAttr']('readonly',!0);inst._input.removeData(inst.name)},enable:function(elem){elem=$(elem);if(!elem.hasClass(this._getMarker())){return}
var nodeName=elem[0].nodeName.toLowerCase();if(nodeName.match(/input|textarea/)){elem.prop('disabled',!1).siblings('button.'+this._triggerClass).prop('disabled',!1).end().siblings('img.'+this._triggerClass).css({opacity:'1.0',cursor:''})}else if(nodeName.match(/div|span/)){elem.children('.'+this._disableClass).remove();this._getInst(elem)._mainDiv.find('button').prop('disabled',!1)}
...
var nodeName=elem[0].nodeName.toLowerCase();if(nodeName.match(/input|textarea/)){elem.prop('disabled',!0).siblings('button.'+this._triggerClass).prop('disabled',!0).end().siblings('img.'+this._triggerClass).css({opacity:'0.5',cursor:'default'})}else if(nodeName.match(/div|span/)){var inline=elem.children('.'+this._inlineClass);var offset=inline.offset();var relOffset={left:0,top:0};inline.parents().each(function(){if($(this).css('position')==='relative'){relOffset=$(this).offset();return!1}});elem.prepend('<div class="'+this._disableClass+'" style="width: '+inline.outerWidth()+'px; height: '+inline.outerHeight()+'px; left: '+(offset.left-relOffset.left)+'px; top: '+(offset.top-relOffset.top)+'px;"></div>');this._getInst(elem)._mainDiv.find('button').prop('disabled',!0)}
...
var inst=plugin._getInst(elem);plugin.hide(null,'');plugin._lastField=elem;plugin._pos=plugin._findPos(elem);plugin._pos[1]+=elem.offsetHeight;var isFixed=!1;$(elem).parents().each(function(){isFixed=isFixed||$(this).css('position')==='fixed';return!isFixed});var offset={left:plugin._pos[0],top:plugin._pos[1]};plugin._pos=null;inst._mainDiv.stop(!0,!0).css({position:'absolute',display:'block',top:'-1000px',width:'auto'});plugin._updateKeypad(inst);offset=plugin._checkOffset(inst,offset,isFixed);inst._mainDiv.css({position:(isFixed?'fixed':'absolute'),display:'none',left:offset.left+'px',top:offset.top+'px'});var duration=inst.options.duration;var showAnim=inst.options.showAnim;var postProcess=function(){plugin._keypadShowing=!0};if($.effects&&($.effects[showAnim]||($.effects.effect&&$.effects.effect[showAnim]))){var data=inst._mainDiv.data();for(var key in data){if(key.match(/^ec\.storage\./)){data[key]=inst._mainDiv.css(key.replace(/ec\.storage\./,''))}}
inst._mainDiv.data(data).show(showAnim,inst.options.showOptions||{},duration,postProcess)}else{inst._mainDiv[showAnim||'show']((showAnim?duration:0),postProcess)}
if(inst._input[0].type!=='hidden'){inst._input[0].focus()}
plugin._curInst=inst},_updateKeypad:function(inst){inst._mainDiv.empty().append(this._generateHTML(inst)).removeClass().addClass(inst.options.keypadClass+(inst.options.useThemeRoller?' ui-widget ui-widget-content':'')+(inst.options.isRTL?' '+this._rtlClass:'')+' '+(inst._inline?this._inlineClass:this._mainDivClass));if($.isFunction(inst.options.beforeShow)){inst.options.beforeShow.apply((inst._input?inst._input[0]:null),[inst._mainDiv,inst])}},_checkOffset:function(inst,offset,isFixed){var pos=inst._input?this._findPos(inst._input[0]):null;var browserWidth=window.innerWidth||document.documentElement.clientWidth;var browserHeight=window.innerHeight||document.documentElement.clientHeight;var scrollX=document.documentElement.scrollLeft||document.body.scrollLeft;var scrollY=document.documentElement.scrollTop||document.body.scrollTop;var width=0;inst._mainDiv.find(':not(div)').each(function(){width=Math.max(width,this.offsetLeft+$(this).outerWidth(!0))});inst._mainDiv.css('width',width+1);if(inst.options.isRTL||(offset.left+inst._mainDiv.outerWidth()-scrollX)>browserWidth){offset.left=Math.max((isFixed?0:scrollX),pos[0]+(inst._input?inst._input.outerWidth():0)-(isFixed?scrollX:0)-inst._mainDiv.outerWidth())}else{offset.left=Math.max((isFixed?0:scrollX),offset.left-(isFixed?scrollX:0))}
if((offset.top+inst._mainDiv.outerHeight()-scrollY)>browserHeight){offset.top=Math.max((isFixed?0:scrollY),pos[1]-(isFixed?scrollY:0)-inst._mainDiv.outerHeight())}else{offset.top=Math.max((isFixed?0:scrollY),offset.top-(isFixed?scrollY:0))}
...
if(this._keypadShowing){inst._mainDiv.stop(!0,!0);duration=(typeof duration!=='undefined'&&duration!==null?duration:inst.options.duration);var showAnim=inst.options.showAnim;if($.effects&&($.effects[showAnim]||($.effects.effect&&$.effects.effect[showAnim]))){inst._mainDiv.hide(showAnim,inst.options.showOptions||{},duration)}else{inst._mainDiv[(showAnim==='slideDown'?'slideUp':(showAnim==='fadeIn'?'fadeOut':'hide'))](showAnim?duration:0)}}
...
this._curInst=null},_doKeyDown:function(event){if(event.keyCode===9){plugin.mainDiv.stop(!0,!0);plugin.hide()}},_checkExternalClick:function(event){if(!plugin._curInst){return}
...
if(elem.setSelectionRange){if(input.is(':visible')){elem.setSelectionRange(pos,pos)}}else if(elem.createTextRange){range=elem.createTextRange();range.move('character',pos);range.select()}},_getIERange:function(elem){elem.focus();var selectionRange=document.selection.createRange().duplicate();var beforeRange=this._getIETextRange(elem);beforeRange.setEndPoint('EndToStart',selectionRange);var checkCRLF=function(range){var origText=range.text;var text=origText;for(;;){if(range.compareEndPoints('StartToEnd',range)===0){break}else{range.moveEnd('character',-1);if(range.text===origText){text+='\r\n'}else{break}}}
...
if(inst.options.randomiseAll){others.push(keys[j])}else if(inst.options.isNumeric(keys[j])){numerics.push(keys[j])}else if(inst.options.isAlphabetic(keys[j])){alphas.push(keys[j])}else{others.push(keys[j])}}}

...
} = require('../../../src/constants/sales.constants');
...
describe('sales.constants', () => {
  test('exports message maps', () => {
    expect(ERROR_MESSAGES).toMatchObject({
      ...
      ITEM_REMOVED: 'This item was already removed and can no longer be changed.',
      INVALID_ITEM_ID: 'Enter a valid item ID',
      ...
      FAILED_TO_PROCESS_SALE: 'Could not process the sale. Please try again.',
      UNAUTHORIZED: 'Unauthorized',
      MISSING_BRANCH_AND_DATE_PARAMS:
        'Missing required parameters: branch, starting_date, ending_date',
      ...
      SALES_EXPORTED_UNSUCCESSFULLY: 'Sales exported un',
      ...
      FAILED_TO_RETRIEVE_USER_GRAPHICAL_REPORTS_PREFIX:
        'Failed to retrieve user graphical reports: ',
      ...
      FAILED_TO_RETRIEVE_PENDING_CUSTOMER_REPORT_PREFIX:
        'Failed to retrieve pending customer report: ',
      ...
      RECENT_SALES_NOT_RETRIEVED: 'Could not retrieve recent sales',
    ...
    expect(SUCCESS_MESSAGES).toMatchObject({
      ...
      SALE_ADDED: 'Sale saved',
      SALES_RETRIEVED: 'Sales retrieved',
      SALES_DELETED: 'Sales deleted',
      GET_SUCCESSFULLY: 'Retrieved',
      GET_SUCCESSFULLY_LOWER: 'Retrieved',
      NO_RECORDS_FOUND: 'No records found',
      RECENT_SALES_RETRIEVED: 'Recent sales retrieved',
      PAYMENT_PROCESSED_SUCCESSFULLY: 'Payment processed',
      GENERIC_SUCCESS_LOWER: 'success',
      SALE_HOLD_SUCCESS: 'Sale placed on hold',
      SERVER_IS_RUNNING: 'Server is running',
      RETURN_SALES_UPDATED_SUCCESSFULLY: 'Return Sales updated',
      ...
      SALES_RECEIPT_EMAIL_SUCCESS: 'Sales receipt email sent',
    });
  });

  test('exports constant arrays', () => {
    expect(PAYMENT_MODE_VALUES).toEqual([
      'Cash',
      'Cheque',
      'CreditCard',
      'Pending',
    ...
    expect(SALE_PROCESS_VALUES).toEqual(['Add', 'Edit', 'Hold', 'PartialReturn', 'Return', 'KOT']);

    expect(GRAPH_ALLOWED_SALE_PROCESSES).toEqual(['Add', 'Edit', 'PartialReturn']);
  });
});

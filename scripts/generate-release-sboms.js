...
function readMongoVersion() {
  const batch = fs.readFileSync(
    path.join(ROOT, "src", "download-mongodb.bat"),
    "utf8",
  );
  const match = batch.match(/^set MONGODB_VERSION=([0-9.]+)$/m);
  if (!match)
    throw new Error("download-mongodb.bat does not pin MONGODB_VERSION");
  return match[1];
}
...
function uniqueByRef(components) {
  ...
  for (const component of components) {
    ...
    if (!seen.has(component["bom-ref"]))
      seen.set(component["bom-ref"], component);
  }
...
function mergeDependencies(
  ...
  merged.set(rootRef, {
    ref: rootRef,
    dependsOn: [...new Set(rootDependsOn)].sort(),
  });
...
function buildSbom({
  ...
  if (!apiRoot || !apiRoot["bom-ref"]) {
    throw new Error(
      "npm SBOM does not contain metadata.component with a bom-ref",
    );
  }
  ...
  const mongoComponent = {
    ...
    externalReferences: [
      ...
      {
        type: "vcs",
        url: `https://github.com/mongodb/mongo/tree/r${mongoVersion}`,
      },
    ],
  };
  const electronComponent = {
    ...
    externalReferences: [
      {
        type: "vcs",
        url: `https://github.com/electron/electron/tree/v${electronVersion}`,
      },
    ],
  };
  ...
  return {
    $schema:
      apiBom.$schema || "http://cyclonedx.org/schema/bom-1.5.schema.json",
    bomFormat: "CycloneDX",
    ...
    metadata: {
      ...
      component: {
        ...
        properties: [
          {
            name: "posnic:artifact:architecture",
          ...
          {
            name: "posnic:licensing:scope",
            value:
              "Mixed-license aggregate; each component license applies separately.",
          },
...
function main() {
  const distDir = path.resolve(
    argumentValue(process.argv.slice(2), "--dist", path.join(ROOT, "dist")),
  );
  writeSboms({ distDir });
}

 ...
 *
 * THE API HALF. The canonical copy is src/order-source.js; this exists because
 * the API ships outside the asar archive and cannot require it. A test compares
 * the two tables, so an edit to one that is not made to the other fails a
 * build rather than printing two different words for the same order.
 *
...
function orderSource(sale) {
  ...
  const channel = String((sale && sale.channel) || '')
    .trim()
    .toLowerCase();

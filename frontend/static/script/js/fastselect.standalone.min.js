...
if(data.length===0){$allResults.html('<p class="'+options.noResultsClass+'">'+(typeof options.noResultsText==='function'?options.noResultsText.call(this):options.noResultsText)+'</p>')}else{if(this.options.responseType==='html'){$allResults.html(data)}else{this['generate'+(data[0][options.responseFormat.groupItems]?'GroupedResults':'SimpleResults')](data,$allResults)}}
...
this.resultsOpened=!0},checkDropdownPosition:function(){var flipOnBottom=this.options.flipOnBottom;var offset=typeof flipOnBottom==='boolean'&&flipOnBottom?400:flipOnBottom;var isFlipped=this.$input.offset().top+offset>$document.height();this.$el.toggleClass(this.options.resultsFlippedClass,isFlipped)},documentCancelEvents:function(setup,onCancel){var self=this;if(setup==='off'&&this.closeEventsSetuped){$document.off(this.ens);this.closeEventsSetuped=!1;return}else if(setup==='on'&&!this.closeEventsSetuped){$document.on(this.namespaceEvents('click keyup'),function(e){if(keyMap[e.keyCode]==='escape'||(!$(e.target).is(self.$el)&&!$.contains(self.$el.get(0),e.target)&&$.contains(document.documentElement,e.target))){onCancel?onCancel.call(self):self.hideResults()}});this.closeEventsSetuped=!0}},navigateItem:function(direction){var $currentItem=this.$resultItems.filter(this.focusedItemSelector),maxPosition=this.$resultItems.length-1;if($currentItem.length===0){this.$resultItems.eq(direction==='up'?maxPosition:0).addClass(this.options.focusedItemClass);return}
...
this.hideResults()},hideResults:function(){if(this.resultsOpened){this.resultsOpened=!1;this.$el.removeClass(this.options.resultsOpenedClass);this.$input.trigger('closingResults');this.documentCancelEvents('off')}
...
self.getOptions(function(){self.renderOptions(!0)})}},getOptions:function(onDone){var options=this.options,self=this,params={};if(this.hasCustomLoader){var query=$.trim(this.$queryInput.val());if(query&&options.apiParam){params[options.apiParam]=query}
...
if(this.userOptionAllowed){var queryInList=this.optionsCollection.models&&this.optionsCollection.findWhere(function(model){return model.value===query});query&&!queryInList&&data.unshift({text:query,value:query,isUserOption:!0})}
